...
function Dialog({open, initialNote, closeDialog, postNote: postNoteState, patchNote: patchNoteState}) {
    ...
    const patchNote = () => {
        try {
            const data = JSON.stringify({title: initialNote.title, content: initialNote.content})
            fetch(`http://localhost:4000/patchNote/${initialNote._id}`, 
            {method: "PATCH",
            headers: {
                "Content-Type": "application/json",
              }, 
            body: data}).then(async function(response)
            {
              if(!response.ok)
              {
              ...
              else
              {
                ...
                patchNoteState(initialNote._id, initialNote.title, initialNote.content)
              }
            })
          }
          catch (error)
